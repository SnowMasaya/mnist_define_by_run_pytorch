...
def resize_data(data):
    ...
    for d in data:
        npimg = to_pil(d)
        scale = transforms.Scale(npimg.width*2)
        npimg = scale(npimg)
        npimg = np.array(npimg)
        np_array.append(npimg)
    np_array = np.array(np_array)
    np_array = np_array.reshape(np_array.shape[0], 1, np_array.shape[1], np_array.shape[2])
